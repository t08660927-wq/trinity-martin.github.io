...
(function () {
    ...
    function initScript() {
        ...
        window.updateCartUI = function () {
            ...
            if (cart.length === 0) {
                ...
                <button data-action="remove" data-id="${p.id}" style="color:#d33;background:none;border:0;cursor:pointer;font-size:18px">×</button>
              `;
        ...
        function openAuth(showRegisterForm = false) {
            ...
            if (document.getElementById('authTitle')) document.getElementById('authTitle').textContent = showRegisterForm ? 'Create account' : 'Welcome — Login';
    ...
    const closeSellerBtn = document.getElementById('closeSeller');
    if (closeSellerBtn && sellerBackdrop) {
    ...
    if (footerSellerLink && sellerBackdrop) {
        footerSellerLink.addEventListener('click', (e) => {
            e.preventDefault();
            sellerBackdrop.style.display = 'block';
            sellerBackdrop.setAttribute('aria-hidden', 'false');
        });
    }
